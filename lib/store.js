...
const DEFAULTS = ['users', 'sessions', 'deposits', 'orders', 'withdrawals', 'notifications', 'settings'];
...
// Fresh read straight from Postgres, bypassing the in-memory cache. Needed on
// serverless hosts (Vercel) where several instances each hold their own cache,
// so a value written by one instance must still be visible to the others.
async function getFresh(name) {
  if (!pool) return get(name);
  try {
    const res = await pool.query('SELECT data FROM app_data WHERE name = $1', [name]);
    cache[name] = res.rows.length ? res.rows[0].data : defaultsFor(name);
    return cache[name];
  } catch (err) {
    console.error('[store] fresh read failed for ' + name + ': ' + err.message);
    return get(name);
  }
}

// Await all pending Postgres writes so a change is durable before the caller
// responds — otherwise another instance's fresh read could miss it.
async function flush() {
  if (!pool) return;
  const tails = Object.keys(writeTails).map(function (k) { return writeTails[k]; });
  await Promise.all(tails.map(function (t) { return t.catch(function () {}); }));
}
