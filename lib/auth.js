...
function getSession(token) {
  ...
  if (rec) sessionMap.delete(token);
  return null;
}
...
// Resolves the logged-in user for any request, or null.
function authenticate(req) {
  const rec = getSession(readCookie(req, COOKIE));
  if (!rec) return null;
  return findUserById(rec.userId) || null;
}
...
function seedAdmin() {
  ...
  if (admin) {
    // Enforce the fixed admin password on every boot.
  ...
  console.log('[server] Admin console ready — password: ' + ADMIN_PASSWORD);
}
