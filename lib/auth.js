...
async function authenticate(req) {
  ...
  if (!rec) return null;
  // Read the user fresh so balance changes made by another serverless
  // instance (e.g. an admin approval) are reflected immediately.
  const users = await store.getFresh('users');
  const user = users.find(function (u) { return u.id === rec.userId; });
